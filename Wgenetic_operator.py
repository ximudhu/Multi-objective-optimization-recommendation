...
def Wgenetic_operator(population, pc, pm, wratmatrix, object_num, tournament_size,predictedlist,a,b):
    ...
    for n in range(population_size):
        ...
        parent1 = tournament_selection (population, object_num, tournament_size)
        parent2 = tournament_selection(population, object_num, tournament_size)
        parent3 = tournament_selection(population, object_num, tournament_size)
        if rand[n] < pc :
            parent1, parent2,parent3 = parent1[0], parent2[0],parent3[0]
            mu_child = np.zeros((user_num, rem_num))
            for i in range(user_num):
                #print(parent1[i],parent2[i])
                com_list = parent1[i].tolist() + parent2[i].tolist() + parent3[i].tolist()
                ...
                if len(count) < rem_num:
                    ...
                    mu_child[i] = count[:rem_num]#random.sample(count,rem_num)
        else:
            mu_child = random.choice([parent1[0],parent2[0],parent3[0]])#,parent3[0],parent4[0]
